<template>
    <div class="box">
        <h1 class="left-align">Step3: 交叉型知识挖掘结果展示</h1>
        <mining-record-card class="record"></mining-record-card>
        <span>
            根据以上挖掘计划，挖掘交叉型知识结果如下。可以选择相应数据，进一步生成四元组知识。
        </span>
...
import { crossMiningResultType } from "@/options.js"; // 导入选项数据
...
export default {
    ...
    computed: {
        ...
        fields() {
            const fieldsArray = [];
            for (let i = 0; i < crossMiningResultType.length; i++) {
                const typeObj = crossMiningResultType[i];
                const field = {
                    name: typeObj.name,
        ...
        selectedTypes() {
            ...
            var selectedTypes = crossMiningResultType
    ...
    methods: {
        ...
        handleChildEvent() {
            ...
            this.$http
                .post("/mining/cross/generate/knowledge", params)
                .then((response) => {
                    // 处理成功响应的数据
